...
class ResNeXtBottleNeck_2d(nn.Module):
	def __init__(self, channels, ker, cardinality=1, dropRate=0):
		...
		self.conv_conv = nn.Conv2d(inter_channels, inter_channels, kernel_size=(ker,ker), padding=(get_padding(ker),get_padding(ker)),
...
class BasicResBlock_2d(nn.Module):
	def __init__(self, planes, ker, n_layers, decode=False, dropRate=0):
		...
		for i in range(n_layers):
			...
			layers.append(nn.Conv2d(planes, planes, (ker,ker), padding=(get_padding(ker),get_padding(ker)), bias=False))
