...
if do_3_stage_train:
    ...
    cfg = {"n_channels": data_loader.N_CHANNELS_OUT, "spk_length": data_loader.N_SAMPLES,
           ...
           "learn_rate": 1e-3, "weight_decay": 1e-6,
    ...
    factors = [60]
    LRs = [1.5e-5,3e-5,6e-5,3e-6]
    conv2_chs = [ 16,32,64]
    # train
    for conv2_ch in conv2_chs:
        for fact in factors:
            for lr in LRs:
                cfg["conv2_ch"] = conv2_ch
                vae_model = Vae(cfg)
                ...
                plt.savefig(os.path.join(os.getcwd(),
                                         'vaeStage3_LD{}_LR{:.0E}_WD{:.0E}_SD{}_DR{:.1f}_F{}_FMC{}.png'.format(cfg["latent_dim"],
                                                                                               lr,
                                                                                               cfg["weight_decay"],
                ...
                vae_model.save_model(os.path.join(os.getcwd(),
                                                  'vaeStage3_LD{}_LR{:.0E}_WD{:.0E}_SD{}_DR{:.1f}_F{}_FMC{}.pt'.format(
                                                      cfg["latent_dim"],
                                                      lr,
                                                      cfg["weight_decay"],
                                                      cfg["shuffle_channels"],
