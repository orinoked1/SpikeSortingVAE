...
for file in gt_files:
    ...
    dict_var = createDict('name', 'train', 'stage', 'include_0', 'architecture', 'LD', 'LR', 'DO', 'F', 'C', 'FLC',
                          'FMC',
...
for file in file_list:
    ...
    if file.name.find("_F") > 0 and file.name.find("_F") != file.name.find("_FLC"):
        ...
        F = 0
    if file.name.find("_FLC") > 0:
        FLC = file.name[file.name.find("_FLC") + 4:]
        if FLC.find("_") > 0:
            FLC = float(FLC[:FLC.find("_")])
        else:
            FLC = float(FLC[:FLC.find(".pt")])
    else:
        FLC = 256

    if file.name.find("_FMC") > 0:
        FMC = file.name[file.name.find("_FMC") + 4:]
        if FMC.find("_") > 0:
            FMC = float(FMC[:FMC.find("_")])
        else:
            FMC = float(FMC[:FMC.find(".pt")])
    else:
        FMC = 16
